...
export default function ExpressPage() {
    const [expressCompetitors, setExpressCompetitors] = useState<BattlecardData[]>([])
    ...
    useEffect(() => {
        fetch('/api/competitors')
            ...
            .then((data: BattlecardData[]) => {
                const sorted = data.filter(c => c.productType === 'Express').sort((a, b) => (a.order ?? 99) - (b.order ?? 99));
                setExpressCompetitors(sorted);
                setLoading(false);
            });
    }, []);

    return (
        <main className="min-h-screen bg-slate-950 text-slate-50 p-6 selection:bg-amber-500/30">
            <div className="max-w-md mx-auto space-y-6 mt-8">
                ...
                >
                    ...
                    <div className="flex items-center gap-3 mb-2">
                        ...
                        <div>
                            <h1 className="text-2xl font-bold tracking-tight">SpotOn Express</h1>
                ...
                ) : expressCompetitors.length === 0 ? (
                    <div className="text-center py-12 text-slate-500 text-sm">No competitors found.</div>
                ) : (
                    <div className="space-y-3">
                        {expressCompetitors.map((comp, idx) => (
                            <motion.div
                                key={comp.id}
                                ...
                                <Link href={`/competitor/${comp.id}`}>
                                    <div className="group relative overflow-hidden rounded-2xl border border-slate-800 bg-slate-900/50 p-5 transition-all hover:bg-slate-800 hover:border-slate-700 active:scale-[0.98]">
                                        <div className="flex items-center justify-between">
                                            ...
                                            <div className="h-9 w-9 rounded-full bg-slate-800 flex items-center justify-center group-hover:bg-amber-500 transition-colors">
